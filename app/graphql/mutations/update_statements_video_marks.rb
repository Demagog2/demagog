...
module Mutations
  class UpdateStatementsVideoMarks < GraphQL::Schema::Mutation
    ...
    def resolve(id:, statements_video_marks_input:)
      ...
      statements =
        statements_video_marks_input.map do |statements_video_mark_input|
          statement = Statement.find(statements_video_mark_input[:statement_id])

          if statement.statement_video_mark.nil?
            statement.statement_video_mark =
              StatementVideoMark.new(statement_id: statement.id, source_id: id)
          end
